...
# Log  
  lg_POP <- dlply(df_i, "Race", function(df_i) 
    lm(Population ~ log(Year-2009), data = df_i))
  par_lg_POP <- ldply(lg_POP, coef)
  lg_HH <- dlply(df_i, "Race", function(df_i) 
    lm(Householder ~ log(Year-2009), data = df_i))
  ...
  df_i <- df_i %>% mutate(
    ...
    POP_log = case_when(Race == "All"~ log(Year-2009)*par_lg_POP[1,3] + par_lg_POP[1,2],
                           Race == "Asian"~ log(Year-2009)*par_lg_POP[2,3] + par_lg_POP[2,2],
                           Race == "Black"~ log(Year-2009)*par_lg_POP[3,3] + par_lg_POP[3,2],
                           Race == "Hispanic"~ log(Year-2009)*par_lg_POP[4,3] + par_lg_POP[4,2],
                           Race == "NH"~ log(Year-2009)*par_lg_POP[5,3] + par_lg_POP[5,2],
                           Race == "NH_White"~ log(Year-2009)*par_lg_POP[6,3] + par_lg_POP[6,2],
                           Race == "Other"~ log(Year-2009)*par_lg_POP[7,3] + par_lg_POP[7,2],
                           Race == "White"~ log(Year-2009)*par_lg_POP[8,3] + par_lg_POP[8,2]
                           ),
    ...
    HH_log = case_when(Race == "All"~ log(Year-2009)*par_lg_HH[1,3] + par_lg_HH[1,2],
                           Race == "Asian"~ log(Year-2009)*par_lg_HH[2,3] + par_lg_HH[2,2],
                           Race == "Black"~ log(Year-2009)*par_lg_HH[3,3] + par_lg_HH[3,2],
                           Race == "Hispanic"~ log(Year-2009)*par_lg_HH[4,3] + par_lg_HH[4,2],
                           Race == "NH"~ log(Year-2009)*par_lg_HH[5,3] + par_lg_HH[5,2],
                           Race == "NH_White"~ log(Year-2009)*par_lg_HH[6,3] + par_lg_HH[6,2],
                           Race == "Other"~ log(Year-2009)*par_lg_HH[7,3] + par_lg_HH[7,2],
                           Race == "White"~ log(Year-2009)*par_lg_HH[8,3] + par_lg_HH[8,2]
                           ),
...
for (i in county_list){
  ...
  p.POP <- ggplot(df %>% filter(Year < 2020), aes(x = Year, y = Population)) +
      ...
      geom_line(data = df, aes(x = Year, y = POP_mixed, color = Race), linetype = "dashed") +
      geom_point(aes(color = Race))

  p.HH <- ggplot(df %>% filter(Year < 2020), aes(x = Year, y = Householder)) +
  geom_point(aes(color = Race)) +
  geom_line(aes(color = Race)) +
  geom_line(data = df, aes(x = Year, y = HH_mixed, color = Race), linetype = "dashed") +
  geom_point(aes(color = Race))
  ...
  ggsave(paste0(df$County, ".png"), device = 'png', 
         path = "Projection/Using_2010_to_2019/mixed")
}
