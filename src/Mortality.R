...
#load("Output/PopData.Rdata") #OLD, do not use!
load("Output/Age_0_4_Freq.Rdata") #AGE_0_4_FREQ
...
SSA <- read_excel("Input/SSA.xlsx") #Source: Table by DE-R (see Documentation), SSA forecast values from 2019 Annual Trustees Report
...
Mort_Proj <- LifeTable %>%
  ...
  mutate(across(starts_with("2"), .fns = ~.*Sx)) %>%
  #select(-Sx)
  rename("2018" = Sx) #keep the calculated Sx

# Fix values > 1  ------------------------------------ This could use some adjustments to make it more dynamic
...
Mort_Proj <- Mort_Proj %>% mutate('Mort2022.5'=rowMeans(across('2020':'2025')),
                                      ...
                                      select(!starts_with("2") | starts_with("Mort"))
