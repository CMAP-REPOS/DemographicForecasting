...
BaseYears_NM <- NM_By_Sex %>%
  filter(Age != "TEMP") |> #ab added
  select(-Age) %>%
  group_by(Region, Sex, Period) %>%
  mutate(NetMigration = sum(NetMigration)) %>%
  distinct() %>%
  group_by(Sex, Region) %>%
  mutate(Sum_BaseYears = sum(NetMigration)) %>%
  select(-NetMigration, -Period) %>%
  distinct()%>%
  group_by(Region) %>%
...
#temp5 <- BaseYears_NM -- other file was blank first time running
...
Sum_NM_Sex <- NM_By_Sex %>%
  ...
  mutate(Sum_NM = sum(NetMigration)) %>%
  select(-NetMigration) %>%
  mutate(Period = 'Base Years') %>%
  distinct() |>
  filter(Age != "TEMP")
...
sum_expectedPop <- expectedpop %>%
  ...
  rename(Age = Age_Group) %>%
  group_by(Region, Sex, Age) %>%
  mutate(ProjectedPop = sum(na.omit(ProjectedPop))) %>%
  distinct()
