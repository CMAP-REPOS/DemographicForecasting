...
load("Output/International_mig_proj.Rdata")
...
pep_year <- as.character(most_recent_PEP)
...
if(startyr == baseyr){
  ...
  Base_Mig <- five_year_combined$x2025 |> group_by(age_group, sex, Region) |> summarize(intl_mig_proj = sum(new_immigrants))
  ...
  Base_Mig <- five_year_combined[[startyr]]
